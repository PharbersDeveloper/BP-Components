...
import layout from '../templates/components/bp-modal';
import TransitionSupport from '../mixins/transition-support';
...
/**
 ...
 ```hbs
 {{#bp-modal onSubmit=(action "submit") as |modal|}}
   {{#modal.header}}
...
export default Component.extend(TransitionSupport, {
	 ...
	 * @private
	 */
	dialogComponent: 'bp-modal/dialog',
	 ...
	 * @private
	 */
	headerComponent: 'bp-modal/header',
	 ...
	 * @private
	 */
	footerComponent: 'bp-modal/footer',
	...
	isFastBoot: computed(function () {
		...
		fastboot = owner.lookup('service:fastboot');
	...
	actions: {
		...
		submit() {
			...
			let modalId = this.get('modalId'),
				forms = this.get('modalElement').querySelectorAll(`#${modalId} .modal-body form`);
	...
	takeFocus() {
		let modalEl = this.get('modalElement'),
			focusElement = modalEl && modalEl.querySelector('[autofocus]');
